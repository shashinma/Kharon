...
public class PowerShellRunner
{
    ...
    public static void HideConsoleWindow()
    {
        try
        {
            ...
            // Redirecionar output para null para evitar qualquer saída
            TextWriter nullWriter = new StreamWriter(Stream.Null);
            Console.SetOut(nullWriter);
            Console.SetError(nullWriter);
        }
        catch
        {
            // Silenciosamente falhar se não conseguir esconder
        }
    }
    ...
    static void Exec(Object stateinfo)
    {
        try
        {
            // Chamar a função para esconder o console ANTES de qualquer coisa
            HideConsoleWindow();
